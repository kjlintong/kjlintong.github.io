...
> 上一篇 [GPU 为什么会挨饿](/blog/mlsysim-system-optimization/) 把视角从单点分析推到全链路优化。今天进入 Datawhale [mlsysim 学习活动](https://github.com/datawhalechina/llm-algo-leetcode/issues/137)的第五阶段,问题换了一句话:**从一台机器走到一千张卡,成本会怎么变?** 对应教程:[06 Scaling to 1000 GPUs](https://harvard-edge.github.io/cs249r_book_dev/mlsysim/tutorials/06_scaling_1000_gpus.html)、[07 Geography is a Systems Variable](https://harvard-edge.github.io/cs249r_book_dev/mlsysim/tutorials/07_geography.html)、[08 The $9M Question](https://harvard-edge.github.io/cs249r_book_dev/mlsysim/tutorials/08_nine_million_dollar.html)。今天跑下来有两个意外:规模扫描那张表里,**通信开销不是随规模上升,而是从 1336.8ms 掉到 319.1ms**;而可靠性那条被重点宣传的结论,交叉点位置比教程说的远得多。

---

## 一、先说结论

这一阶段跑的是 E1(3D 并行)、E2(规模扫描)、O1(可靠性与 checkpoint)、E4(选址)、E3(经济学),全程 Llama-3-70B + DGX H100 + InfiniBand NDR,mlsysim 0.1.2,纯仿真。五条值得单独记住的:

1. 千卡训练最常被讲的瓶颈是通信。实测里通信占比只有 5.6%~7.4%,而且**随规模上升不升反降**——但这不是通信变好了,是教程那张表里混入了 batch 口径的变化。
2. 引擎的集群 MTBF 是**节点级串联模型**,单节点 4285.7h,512 卡 67.0h。教程背景栏里"512 卡约 97h"是按 GPU 数直除 MTTF 得来的另一个假设。
3. 教程 §5 说"可靠性开销主导通信开销"。方向对,但按它的口径,**交叉点在 2560 卡附近,不在 1024 卡**;1024 卡时可靠性开销只有通信的 0.71 倍。
4. 同一个 30 天作业,引擎里其实有两套可靠性账本。§5 用的是"写入 + 半个间隔回滚";`ReliabilityResult.goodput_ratio` 用的是"写入/间隔 + 修复时间/MTBF",在 1024 卡上给出的开销只有 §5 的 1/2.9。
5. 选址是这一阶段最强的单一杠杆:同样 32 卡、同样 30 天,波兰比魁北克多 61.1 倍碳。而经济学这边,30 天窗口里 Capex 恒占 83.4%——**短期训练的钱几乎全是折旧,不是电费**。

---

## 二、E2 规模扫描:那张表为什么会反着走

教程给的扫描很直白:TP 固定为 8(节点内 NVLink),PP=1(不引入气泡),DP 靠节点数自然扩大,batch 取 `max(64, n_gpus)`:
...
通信从 1336.8ms 降到 236.2ms,再爬回 319.1ms。扩展效率全程贴在 92%~94%,看起来规模扩展近乎完美。教程的解释是 Amdahl 定律下的通信开销"可控",然后话锋一转去讲可靠性。
...
也就是说,8 卡和 1024 卡的"通信"根本不是同一个东西:前者是一个节点里 8 张卡为了 64 个样本互相交换激活值,后者是 128 个节点同步梯度。**两个量被同一列数字记录,才产生了"规模越大通信越少"的观感**。换成固定 batch 重扫,结论正常:
...
通信随规模单调上升,但天花板很低(319ms,占 7.4%)。这个结论本身是对的——教程的"通信可控"站得住——**只是它的论证路径不干净**。
...
四个点完全重合,P=4、P=8 也一致。引擎用的是理论式的朴素版:没有 1F1B 的额外收益,也没有交错流水线的压缩。所以**引擎里的微批次只能换来"公式里的改善",换不来调度算法的工程收益**——这点在拿它做并行度决策时要记住。
...
三个规律很干净:**PP 一启用,气泡直接吃掉一半或 3/4**;TP 从 1 到 4 效率单调升(内存墙压力变小);TP=8 反而回落。

但有个前提被忽略了:以上 9 种配置的 `node_profile.feasible` **全部为 False**。70B 模型加 Adam 状态,单卡显存需求 2192~10172 GiB,远超 85.9 GiB。引擎照样返回效率和吞吐,因为这些量是按"假设数据能放下"算的。所以这张表能告诉你**给定能跑的前提下 TP/PP 怎么权衡**,不能告诉你"8 卡够不够"——后者答案是明确的不够。

---

## 三、O1 可靠性:Young-Daly 公式该往哪用

教程背景栏写:单卡 MTTF 50,000h,512 卡集群约 97h。这是按 `MTTF / N` 直接算的。但引擎的 `ReliabilityModel` 不是这么实现的——它先算节点,再算集群:
...
单节点 8 GPU + 8 网卡 + 2 电源,代入 MTTF 50,000 / 150,000 / 100,000h:
...
8 个 writer 之后曲线开始弯,64 之后基本压到 2s 不动——**写盘有一个硬件上限**(引擎按 500 GB/s 文件系统上限 + 有效带宽折算,4 个 writer 就是 35.3s,说明上限在 4 左右开始生效)。所以"多写盘通道"是一笔真实但很快耗尽的收益,不是无限杠杆。
...
2. **交叉点在 2560 卡附近,不在 1024 卡**。教程的结论句"At scale, reliability overhead dominates communication overhead"方向没错,但按它自己给的表(1024 卡时 0.71×),dominates 这个动词用得早了 2 倍规模。

而引擎内部还有第三个数字。`ReliabilityResult.goodput_ratio` 用的公式是 `1 − C/M − R/MTBF`(C 是单次写盘,R 是修复时间 300s),这是**稳态占比**,分母已包含 §5 那 7.4% 的通信损失,回滚也只计 300s/次而不是半个间隔:
...
值得记的一件事:**§5 口径下,写入和回滚是严格相等的**(每行 write=rollback)。原因是 Young-Daly 的最优间隔恰好让两项各占一半——`C·(T/C)=1` 与 `f·(T/2)=1` 在最优解处同时成立。所以"checkpoint 开销"这个词在这个模型里天然意味着"写入 + 同量回滚",不是两个独立项。反过来也印证了上面的 C 陷阱:如果把最优间隔用默认 C=60s 算、而实际写盘仍按 141.2s,这一等式立刻打破(64 卡:写入 6.7h vs 回滚 2.8h)——间隔被压得太短,checkpoint 次数变多,写入反超回滚。
...
波兰/魁北克 = **61.1 倍**,波兰/美国 = 2.7 倍,波兰/冰岛 = 43.7 倍。教程 07 反复强调的"40 倍"是**纯度差**(820/20 gCO₂/kWh),实际碳差还叠加了 PUE(1.58 vs 1.06)和能耗结构差异(25.5 vs 17.1 MWh)——所以真实数字比纯度差更大。

这里有个反直觉的点:教程 §2 把 30 天魁北克和 10 天波兰对比,结论是"波兰用 1/3 的时间就比魁北克完整跑一次更脏"。算术上成立(7.0 t > 0.3 t),但**这句话的杠杆其实是纯度而不是时长**——30 天魁北克 vs 15 天波兰同样是波兰更脏(4.1 t vs 0.3 t)。换句话说,只要纯度差远大于时长比,时长这个变量几乎可以忽略。选址决策真正的意义是:**它是一次性的、永久性的**,而训练时长是任务决定的。
...
右图是另一个墙:写盘。141s → 2s 的收益,几乎全部来自 1→8 个 writer,64 之后曲线压平。所以"加存储带宽"和"调 checkpoint 间隔"是两件性质不同的事:前者是一次性投入换 1% 的 MFU,后者是零成本换 3~7% 的 30 天损失。
...
4. **40 倍还是 61 倍**。纯度差和总碳差是两回事,PUE 会把差距放大 47%。
