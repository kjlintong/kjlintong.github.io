...
description: "MLSys 学习的第三阶段记录:基于 Datawhale mlsysim 打卡任务 Task3(KV Cache 与模型属性),亲手算清 KV-Cache 的显存账本——公式每一项对应 Llama-3-8B 的哪个超参数,实测序列长度翻倍则并发上限减半(2K 260 → 4K 130 → 32K 16 → 128K 4),并推翻了一个常见直觉:INT4 加速比随 batch 单调上升(3.38x→3.86x)而非\"临界点下降\",根因是引擎里 KV-Cache 也随精度缩放。最后用 CompressionModel 实测量化和剪枝:量化\"压缩即加速\",非结构化剪枝\"只省存储不省时间\"。"
date: 2026-09-12
author: Ryan
